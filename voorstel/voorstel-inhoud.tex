...
In een grafiekdatabase slaan we gegevens op in de vorm van vertices (nodes) en relaties (edges), hierdoor kunnen we complexe verbanden leggen zoals in ons geval de productieprocessen.
...
Binnen zo'n grafiekmodel hebben we vertices die service providers, locaties of resources beschrijven zoals in ons geval bijvoorbeeld een slab, een afdeling of een locatie.
De edges zullen de events omschrijven zoals bijvoorbeeld de verwerking van een product door een afdeling.
...
In ons geval kunnen we bijvoorbeeld vragen ``We hebben een klacht over batch 123, kan je me vertellen wat er mogelijk fout is gegaan?'', en op dat moment vraagt het LLM aan RAG alle informatie over die batch waarna hij een geschikt antwoord kan geven.
...
Een combinatie van deze 2 methodes zou in ons scenario ideaal zijn. Dit omdat ArcelorMittal veel vakjargon heeft zoals cokes, slabs,\dots waarmee we ons model kunnen specialiseren en de data die we gebruiken specifiek en up-to-date moet zijn om van elk moment op de hoogte te blijven.

\subsection{EPCIS}
Voor dit onderzoek moet alles voldoen aan de EPCIS (Electronic Product Code Information Services) standaard, dit zijn de wat, wanneer, waar, waarom en hoe. 
...
Door deze normen toe te passen kunnen we de traceerbaarheid van het product per proces garanderen inclusief de sensor report die opgeslagen wordt in ons grafiekmodel zoals tijd (wanneer) en temperatuur (hoe) waar nodig.
\subsubsection{Waarom EPCIS?}
Huidige Legacy Systemen maken gebruik van ERP, MES, WMS, DAM, \dots, via EPCIS daarentegen kunnen we onder andere adoptatie eenvoudiger laten verlopen. Denk aan een bedrijf dat kleinere bedrijven heeft opgekocht en een overzicht wil hebben over de verschillende sites heen.
Als voorbeeld hebben we Tailored Blanks, dit is een entiteit die los staat van ArcelorMittal Gent, maar wel op dezelfde site zit. Hierbij hebben ze hun eigen IT systemen en personeel die los staan van elkaar, maar door de EPCIS standaard kunnen deze eenvoudiger informatie uitwisselen om inzicht te krijgen.
Ook bij de supply chain geldt dit principe, zo hebben we bijvoorbeeld blanks, lasergelaste vormstukken van bijvoorbeeld voertuigen die kant en klaar zijn voor autobouwers en op een returnable carrier geplaatst worden om te vervoeren \autocite{AMGBlanks}. We willen altijd weten waar die zijn, maar een GPS systeem voor deze carriers zouden te duur worden, maar wat als het ontvangst ons digitaal kan zeggen waar ze zijn op een eenvoudige manier.
...
Door de implementatie van dit systeem hoop ik dat we klanttevredenheid en besparingen binnen een bedrijf (in ons geval ArcelorMittal Gent) kunnen optimaliseren
\begin{figure}[h!]
    \centering
